...
<select className='border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-purple-500 outline-none transition w-full sm:w-auto'
...
className='bg-purple-50 border border-purple-200 shdow-sm rounded-2xl p-6 text-center mb-10'
>
<h2 className='text-lg font-semibold text-purple-700'>{title}</h2>
<p className='text-4xl font-extrabold text-purple-800'>₹{currentEarning.toLocaleString()}</p>
</motion.div>
<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10'>
{stats.map((s,i)=>{
    const icons=[
        <Package key="p" className='text-purple-700 w-6 h-6'/>,
<Users key="u" className='text-purple-700 w-6 h-6'/>,
<Truck key="t" className='text-purple-700 w-6 h-6'/>,
<IndianRupee key="r" className='text-purple-700 w-6 h-6'/>
    ...
    className='bg-white border border-gray-100 shadow-md rounded-2xl p-5 items-center gap-4 hover:shadow:lg transition-all'
    >
...
<ResponsiveContainer width="100%" height={300}>
    <BarChart data={chartData}>
        <CartesianGrid stroke="#acc" strokeDasharray="5 5"/>
        <XAxis dataKey="day"/>
        <Tooltip/>
