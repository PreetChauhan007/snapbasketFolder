...
const router=useRouter()
  ...
  return (
    <div className='w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative'>
      <Link href={"/"} className='absolute -top-2 left-0 flex items-center gap-2 text-purple-700 hover:text-green-800 font-medium transition-all'>
      ...
      className='text-2xl sm:text-3xl md:text-4xl font-bold text-purple-700 text-center mb-10'
      >
       🛒 Your Shopping Cart 
      </motion.h2>
...
<Link href={"/"} className='bg-purple-600 text-white px-6 py-3 rounded-full hover:bg-purple-700 transition-all inline- block font-medium'>Start Shopping
</Link>
</motion.div>
):(
...
<div className='mt-4 sm:mt-0 sm:ml-4 flex-1 text-center sm:text-left'>
    ...
    <p className='text-purple-700 font-bold mt-1 text-sm sm:text-base'>₹{Number(item.price)*(item.quantity)}</p>
</div>
<div className='flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-50 px-3 py-2 rounded-full'>
<button className='bg-white p-1.5 rounded-fullhover:bg-purple-100 transition-all border border-gray-200' onClick={()=>dispatch(decreaseQuantity(item._id))}><Minus size={14} className='text-purple-700'/></button>
<span>{item.quantity}</span>
<button className='bg-white p-1.5 rounded-fullhover:bg-purple-100 transition-all border border-gray-200' onClick={()=>dispatch(increaseQuantity(item._id))}><Plus size={14} className='text-purple-700'/></button>
</div>
...
<div className='space-y-3 text-gray-700 text-sm sm:text-base'>
    ...
    <div className='flex justify-between'>
        ...
        <span className='text-purple-700 font-semibold'>₹{deliveryFee}</span>
    </div>
    <hr className='my-3' />
    <div className='flex justify-between font-bold text-lg sm:text-xl'>
        <span>Final Total</span>
        <span className='text-purple-700 font-semibold'>₹{finalTotal}</span>
    </div>
</div>
<motion.button
whileTap={{scale:0.95}} className='w-full mt-6 bg-purple-600 text-white py-3 rounded-full hover:bg-purple-700 transition-all font-semibold text-sm sm:text-base ' onClick={()=>router.push("/user/checkout")}
>
    Proceed to Checkout
</motion.button>
</motion.div>
