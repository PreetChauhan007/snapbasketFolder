 ...
 const handleSearch=(e:React.FormEvent)=>{
  ...
  <div className='pt-4 w-[95%] md:w-[85%] mx-auto pb-20'>
    ...
    >
        <button
        onClick={()=>router.push("/")}
        className='flex items-center justify-center gap-2 bg-purple-100 hover:bg-purple-200 text-purple-700 font-semibold px-4 py-2 rounded-full transition w-full sm:w-auto'
        >
            
            <ArrowLeft size={18}/><span>Back</span></button>
        <h1 className='text-2xl md:text-3xl font-extrabold text-purple-700 flex items-center justify-center gap-2'><Package size={28} className='text-purple-600'  />Manage Items</h1>
    </motion.div>
    <motion.form
...
<p className='text-gray-500 text-sm capitalize'>{g.category}</p>
    ...
    <div className='mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2'>
        ...
        <button className='bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 hover:bg-purple-700 transition-all' onClick={()=>setEditing(g)}>
            <Pencil size={15}/> Edit
        </button>
    </div>
...
initial={{y:40,opacity:0}}
        ...
        className='bg-white rounded-2xl shadow-2xl w-full max-w-lg p-5 relative '
>
<div className='flex justify-between items-center mb-4'>
    <h2 className='text-2xl font-bold text-purple-700'>Edit Item</h2>
    <button className='text-gray-600 hover:text-red-600' onClick={()=>setEditing(null)}>
        <X size={18}/>
    </button>
</div>
<div className='relative h-64 w-full rounded-lg overflow-hidden mb-4 border border-gray-200 group'>
{imagePreview && <Image
src={imagePreview}
alt={editing.name}
fill
...
/>}
<label htmlFor="imageUpload" className='absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-opacity'><Upload size={28} className='text-purple-500'/></label>
...
<button className='px-4 py-2 rounded-lg bg-purple-600 text-white flex items-center gap-2 hover:bg-purple-700 transition-all '
onClick={handleEdit}
disabled={loading}
>
{loading?<Loader size={14}/>:" Edit Item"}   
</button>
<button className='px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 transition'
onClick={handleDelete}
disabled={deleteLoading}
>
    {deleteLoading?<Loader size={14}/>:"Delete Item"}   
</button>
</div>
